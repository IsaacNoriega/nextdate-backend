...
public class RecommendItineraryService implements RecommendItineraryUseCase {
  ...
  public Itinerary recommend(UUID userId, String prompt) {
    ...
    Profile profile =
        profileRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException("Perfil no encontrado para el usuario " + userId));

    List<SharedExperience> activeExperiences = sharedExperienceRepository.findAllActive();

    String jsonRecommendation =
        aiConciergeClient.generateItineraryJson(profile, activeExperiences, prompt);

    try {
      RecommendItineraryJson parsed =
          objectMapper.readValue(jsonRecommendation, RecommendItineraryJson.class);
                  ...
                  item ->
                      new CreateItineraryUseCase.CreateItemCommand(
                          ...
                          item.transportToNext() != null
                              ...
                              : TransportType.NONE,
                          item.transitTimeToNext()))
              .toList();

      CreateItineraryUseCase.CreateCommand createCommand =
          new CreateItineraryUseCase.CreateCommand(
              userId, parsed.title(), parsed.description(), parsed.totalCost(), true, itemCommands);
  ...
  private record RecommendItineraryJson(
      String title, String description, double totalCost, List<RecommendItemJson> items) {}
